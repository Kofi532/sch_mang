...
import pandas as pd
from .forms import PostForm, RegForm, ActTerm, FeeForm
...
def download3(request):
    ...
    df_act = df_act[df_act['username'] == usernamed]
    term = list(df_act['active_term']) 
    term = term[0]
...
#['stu_id', 'firstname', 'middlename', 'lastname', 'level', 'amount','amountpaid_term1', 'amountpaid_term2', 'amountpaid_term3','fee', 'balance', 'school', 'datey', 'school_full', 'mother_name', 'mother_contact', 'father_name', 'father_contact']
        columns = ['number', 'stu_id', 'firstname' , 'middlename', 'lastname','level', 'fee', 'balance for term','amountpaid_term1', 'amountpaid_term2','amountpaid_term3', 'amount' ]
        for col_num in range(len(columns)):
            f1.set_bold(True)
            worksheet.write(row_num, col_num, columns[col_num], f1) 
            worksheet.set_column(row_num, col_num, 20)
...
def fees(request):
        ...
        if request.method == 'POST'and f_form.is_valid():
            ...
            df =  pd.DataFrame(class_fee.objects.all().values().filter(school = sch))
            class_fee.objects.filter(school =sch).filter(classes= classes).update(fee = fee)
            return render(request, 'thanks.html', {})
        return render(request, 'fee.html', {'f_form': f_form, 'cfees':cfees, 'schf': schf})
